...
for (let i = 0; i < 80; i++) {
    ...
    const sig = info
        ? `${info.tag}#${info.id || ""}.${info.cls || ""}|${info.text}`
        : "NULL";
    if (last && sig === last && i > 5) {
        note(`  -- focus stuck at iter ${i}`);
        break;
    }
    note(`${String(i + 1).padStart(2)}. ${JSON.stringify(info)}`);
    // detect wrap to skip-link
    if (i > 5 && info?.cls?.includes("skip-link")) {
        cycled = true;
        note(`  -- cycled back to skip-link at iter ${i + 1}`);
        break;
    }
    last = sig;
}
...
const before = await page.evaluate(() => ({
    ...
    panels: Array.from(
        document.querySelectorAll('[role="tabpanel"], [id^="day-"]'),
    ).map((p) => ({
        id: p.id,
        hidden: p.hidden,
        hasHiddenAttr: p.hasAttribute("hidden"),
...
const afterRight = await page.evaluate(() => ({
    ...
    panels: Array.from(document.querySelectorAll('[id^="day-"]')).map((p) => ({
        id: p.id,
        hidden: p.hidden,
    })),
}));
...
for (const r of routes) {
    ...
    const btns = await page.$$(
        'button[aria-label="Play video"], button[aria-label="Pause video"]',
    );
    if (btns.length) {
...
const rState = await rp.evaluate(() => {
    ...
    const ariaLabelCount = els.filter((e) =>
        e.hasAttribute("aria-label"),
    ).length;
    const transparentCount = els.filter(
        (e) =>
            window.getComputedStyle(e).color === "rgba(0, 0, 0, 0)" ||
            window.getComputedStyle(e).color === "transparent",
    ).length;
    return {
        total: els.length,
        withAriaLabel: ariaLabelCount,
